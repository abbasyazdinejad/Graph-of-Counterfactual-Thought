...
def force_apply_perturbation(inst, pname: str):
    """
    Safety net: if your perturbations registry misses a case, or the string removal
    is too brittle, we enforce the intended *semantic* perturbation here.

    This keeps CF-eval meaningful: the perturbed evidence should actually change.
    """
    try:
    ...
    # modify_c2 / mask_encryption / inject_c2_backdoor are left to apply_perturbation
    return inst2
...
def run_cva(inst, llm):
    """
    CVA: root (CoT) + CF runs (CoT) with a stronger perturbation application.

    NOTE: If you later add a dedicated CVA prompt that jointly reasons over
    the root + counterfactuals, you can swap this out.
    """
...
def main():
    ...
    # store per-perturbation rows for auditing
    ...
    for inst in instances:
        ...
        r_cot, _, css_cot, flip_cot, acc_o_cot, acc_cf_cot, rows_cot = run_baseline_with_counterfactuals(inst, llm, "CoT")
        r_tot, _, css_tot, flip_tot, acc_o_tot, acc_cf_tot, rows_tot = run_baseline_with_counterfactuals(inst, llm, "ToT")
    ...
    t = Table(title="Step 4 (OpenAI) — Summary (Baselines + CVA with Counterfactual Eval)")
    t.add_column("Method")
    ...
    # Save main CSV
    out_path = OUT_DIR / f"table_metrics_openai_{model_tag}.csv"
    ...
    # Save per-perturbation audit CSV
    per_path = OUT_DIR / f"per_perturbation_openai_{model_tag}.csv"
