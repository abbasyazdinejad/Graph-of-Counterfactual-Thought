...
from cycb.llm_gemini import GeminiChatLLM
...
def run_cva(inst, llm):
    # Same CVA definition you used in OpenAI step4: CoT root + CoT CF runs
...
def main():
    ...
    model_name = (Path.cwd() / ".env").read_text().find("GEMINI_MODEL")
    # use env var; default is fine
    import os
    gemini_model = os.getenv("GEMINI_MODEL", "gemini-2.5-flash-lite")
    temperature = float(os.getenv("GEMINI_TEMPERATURE", "0.0"))
    model_tag = gemini_model.replace("/", "_")

    llm = GeminiChatLLM(model=gemini_model, temperature=temperature)

    methods = ["Direct", "CoT", "ToT", "CVA"]
    results = {m: [] for m in methods}
    per_pert_rows: list[dict] = []

    log_path = OUT_DIR / f"gemini_eval_log_{model_tag}.jsonl"
    with log_path.open("w", encoding="utf-8") as logf:
        for inst in instances:
            ...
            root, cfs, g_cot, rows_cva = run_cva(inst, llm)
    ...
    t = Table(title=f"Step 4 (Gemini: {gemini_model}) — Summary (Baselines + CVA with Counterfactual Eval)")
    t.add_column("Method")
    ...
    print(f"[bold green]Saved JSONL log:[/bold green] {log_path}")
